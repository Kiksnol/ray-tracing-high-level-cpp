...
namespace mth
{
  ...
  public:
    ...
    /* Default constructor */
    camera( VOID ) :
      Loc(1.23, 2.59, 5), Dir(0, 0, -1), Up(0, 1, 0), Right(1, 0, 0), At(0, 0, 0),
    ...
    {
      matr<type> m = matr<type>::Translate(-At) * matr<type>::Rotate(AngleInDegree, Axis) * matr<type>::Translate(At);
    ...
    /* Set ray to point in screen function.
     * ARGUMENTS:
    ...
    {
      type Wp = Size, Hp = Size;

      if (FrameW >= FrameH)
        Wp *= (type)FrameW / FrameH;
      else
        Hp *= (type)FrameH / FrameW;

      vec3<type>
        A = Dir * ProjDist,
        B = Right * (Xs - FrameW / 2.0) * Wp / FrameW,
        C = Up * (FrameH / 2.0 - Ys) * Hp / FrameH,
        Q = A + B + C;
   ...
   /* Useful types */
  //typedef camera<FLT> camera; /* Float */
